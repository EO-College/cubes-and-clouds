...
> "Reliable" needs more explanation - I think mentioning 'criteria out of the professional context' is too vague, is it possible to describe what criteria might be out of the professional context? 
...
> Here you mention "this is the same R we mentioned when we talked about FAIR", but this is the first time FAIR is mentioned in this module. You could just remove the sentence or link to the earlier section on FAIR
...
> In my opinion, this "R" needs more context given on how it explicitly fits into the larger Open Science concept
...
> In the data section above, do you want to specifically recommend not just unique but persistent identifiers? Also, noting that metadata isn't mentioned here, but is very important to findability/reusability of data
> Note that I would personally link to the open licensing course here, and any other mentions like this, when all courses are complete
...
> I'll note here that we also recommend that authors use the Zenodo-Github link or similar when sharing a published version of their code to give the code version a persistent, unique identifier, in addition to sharing on Github
...
> More background or information on metadata may be needed in the next section for the reader/student.
...
> I recommend freezing the last frame of this animation for a few more seconds so the reader can take it all in
...
> I think a word is missing in several places here after "Geospatial"... platforms? analysis? Geospatial is an adjective and needs a noun
...
- **FAIRness:** 
 > Maybe add a sentence here adding the context that this section on FAIR is specifically about how FAIR concepts are integrated in EO Cloud Platforms, to address my comment before that it seems like FAIR is mentioned several times
  ...
  - Accessible: Data access in cloud platforms is usually granted after registration and authentication. Since cloud computing resources are available a certain degree of access control is necessary to prevent misuse of the resources.
> I think above something more is needed after 'available' to make the sentence more meaningful... 'available to all'? 'available openly on the internet'?
...
> is analysis ready data always satellite data, or do you mean in the context of the EO Cloud Platforms? 
