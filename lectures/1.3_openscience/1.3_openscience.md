...
- **FAIRness:** 
  ...
  - Interoperable: The use of cloud native data formats and processing standards like openEO make the code interoperable as well, standardised metadata attached to the results guarantees uptake of the results right away. Different sources of satellite data are made interoperable by the cloud platform through the use of data cubes and processing on the fly - reprojections, regridding and temporal alignment are enabled on the fly.
  - Reusable: License, Link to the results can be shared, as well as the process graph/code to work collaboratively
