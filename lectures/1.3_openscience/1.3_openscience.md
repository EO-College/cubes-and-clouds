...
**Video: Open Science Manifest --> A.A.?**
...
- **Intro:** Hi, let’s go on the open science journey. We will learn how the concepts of open science, FAIR, open data and open-source software are applied in a real research project. We’ll follow along the way from input data, analysis, result generation up to scientific publication. The project ClirSnow is a great example – embracing all the elements of open science.
- **Research Question + ClirSnow Description:** Before we get going. Let’s find out what ClirSnow is about? The project describes the past, present and future snow cover in the European Alps. The results are obtained by combining in-situ observations, remote sensing data, and climate models. The Project is a Post-doctoral fellowship funded by the EU Horizon 2020 Marie Skłodowska-Curie Actions carried out by Michael Matiu.
- **Input Data:** The first step is to gather the input data. Satellite data and climate data come in handy for analyzing snow cover across the alps. Fortunately, the satellite data and climate data are already openly available. Perfect for reproducibility! For the snow depth station data, the ground truth, across different countries in the Alps this is not the case. It is massive manual work to gather these measurements across different countries and provinces – and to harmonize them. There are more than 2000 stations from Austria, Germany, France, Italy, Switzerland, and Slovenia. Keep that dataset in mind!
- **Analysis and Code:** All the analysis needed to (re)produce the results of the project are publicly available on GitHub. Everybody who is interested can download the code and reproduce the results.
Additionally, an R-Package, that make life easier working with daily netcdf files from the EURO-CORDEX RCMs, is published on CRAN, a public software repository. The project has not only shared and documented its code it’s also created a generic piece of open-source software.
- **Results + Data:** Remember the snow station measurement collection mentioned in the beginning? This dataset has been made FAIR and open. The dataset is registered on openAIRE (an open science catalogue) and has its Digital Object Identifier. It’s findable. It’s hosted on ZENODO (an open data portal). So, it’s accessible. It uses machine readable formats (in this case annotated csv files). It’s interoperable. And it has an open license attached to it (CC-BY-4.0). It’s reusable. A great example of open and FAIR data. The FAIR self assessment tool of ARDC (Australian Research Data Commons) gives this data set an almost perfect rating! By the way also the Snow Cover and Snow Cover predictions produced in the project are published under the same conditions.
- **Publication:** Data and code are produced according to open science principles. Finally, the scientific publication is the last step of the open science journey. The paper has been published in an Open Access Journal “The Cryosphere”: The access is open for everybody without paying fees or registering. It’s under the license CC-BY-4.0 Beforehand, it has been made available as a Pre-Print. This allows to follow the review process openly online and to comment on the paper in an early stage. Data and Code are linked to the publication by their identifiers. This increases the reproducibility of the presented results tremendously! All in all, there are no barriers to find this publication, read it, see how other researches think about it and reproduce its results.
- **Wrap-Up:** We’ve covered all steps of the open science journey! Be sure that the journey is not always smooth, and it takes some effort, especially doing it for the first time. But hard work for sake of opening research pays off. Look at the impact the ClirSnow project has made by adhering to open science. The snow station measurement dataset, which has not been available at all before, has already more than 4000 downloads. 4000 people profit from going the last step and making the dataset FAIR and open. Now that you know your way around: it’s your turn to follow the journey for your own research! Special Thanks to Michael Matiu who let us his project to demonstrate the open science journey! Motivation Quote from Michael Matiu: "It seems like a lot of work the first time you do it. And it is. But once you know how to do it, you will use it in every research project, because it actually makes research so much easier. And - it will boost your research impact and credibility. It is really worth it."

### Examples of Open Science in EO 

#### The Role of Open Source Software in Geospatial - The example of GDAL
- **Intro:** Have you ever heard of GDAL? The Geospatial Data Abstraction Library? No, okay, outside of the geoinformatics world few people have heard of it, and even within it – not everybody knows it. Already the name sounds very abstract! But be sure, its role is the opposite of that! In geospatial you are most likely using GDAL every day without knowing! In fact, Howard Butler, a director of the Open-Source Geospatial Foundation, said:  “[…] It's open, it provides core functionality, I can't understand how anybody gets anything done without it.“ (Mark Wilcox (2009). Porting to the Symbian Platform: Open Mobile Development in C/C++. Wiley.)
If it’s so important. There must be something to it. Let’s discover what GDAL is. And what its role in the geospatial world is.
- **Brief History:** GDAL has originated from the original purpose of translating different raster formats (which it still does). Since 1998 it has been constantly growing and evolving by adding a multitude of features and formats to where it is today. It supports more than 160 raster formats, more than 80  vector formats and carries around 50 functions (with 1000s of parameters). This year is the 25th birthday of GDAL! And it is still evolving!
- **GDAL-Open-Source:** What makes GDAL open source. Here are three out of many reasons GDAL is open-source software. In general, GDAL is licensed under an MIT style license with some special terms. As a permissive license, it puts only very limited restriction on reuse. The MIT License also permits reuse within proprietary software. Free redistribution is possible.  GDALs source code is openly available on GitHub. Everybody can access it. Since the license is open and code is available - GDAL has an active community. GDAL has one main developer who is responsible for the library: For over a decade it was the original founder Frank Warmerdam who was then followed by Even Rouault. Still the community is actively contributing to its success. By actively contributing to the software, there are more than 440 contributors. Or by logging issues, there have already been 2400 issues solved.
- **Brief Functionality:** GDAL understands raster and vector formats in a geospatial sense and allows to use them together. This includes the conversion of different formats and the processing of data, like changing resolutions, spatial reference systems or raster calculations. Natively it is used via the command line. Where it really shines though is in enabling other software, where it serves as the backbone. Paul Ramsey, the co-founder of the PostGIS extension, has put it like this in a mapscaping.com podcast: “GDAL is data plumbing, a bit like an international electrical plug set for traveling — it’s got multiple different shaped plugs.
...
At the core, electricity is electrons vibrating, but it can be quite complex to get your hair dryer spinning. In the same way, raster is a bunch of pixels. Each pixel has a value. But there are hundreds of ways to arrange that data on a disk, in files for computers, and to manipulate those pixels to work with as a GIS professional.”
- **Role of GDAL:** As said before, GDAL is the backbone of countless other software solutions. You’d be surprised where you can find it:
Of course, in open-source software like QGIS, GeoServer, but also in big players like Google Earth, in private software like ArcGIS, and in geospatial libraries for programming languages like R and Python. And there are many more! Nearly any “Add raster” function is backed by GDAL. That is because it already translates 160 formats as a single library, a single dependency. And if there’s a new format it will appear in GDAL, and if not: it’s a lot less work to add it to GDAL than to write your own full fledged data model. As you see: GDAL is critical infrastructure for the geospatial domain and far beyond.
- **GDAL and Clouds:** We’ve seen how important GDAL is in the geospatial software environment. But is it also ready for the cloud? Cloud computing is expanding. Software stacks are added to cloud solutions to reduce data transfer. As user numbers grow, the license cost for proprietary software explodes. This is why open-source software so common in cloud computing.  Landsat 8 images are stored on AWS cloud. The pre-processing is done with GDAL. The access to the images in the cloud is also done via GDAL. Relying on a cloud native format, cloud optimized Geotiff. GDAL understands this format, and many other cloud native formats as well. Nearly all EO cloud platforms have GDAL involved somewhere: Google Earth Engine, The Microsoft Planetary Computer, openEO Platform. GDAL plays a key role in EO cloud computing. That’s because it’s free and it adapts quickly to new technologies.
- **OS in Geospatial:** GDAL is just one example. There are so many other Open-Source initiatives in geospatial. In fact, there are so many of them that are so impactful, that organizations have formed to keep track of them and provide networks for collaboration: FOSS4G (Free and Open Source Software for Geospatial), OGC (Open Geospatial Consortium), GEO (Group on Earth Observations), OSGeo (Open Source Geo), to name the most important. The geospatial domain cannot be imagined without Open Source Software.
- **End:** Have you ever heard about GDAL? Now, Yes! It’s hard to forget about GDAL after realizing the role it plays. As we’ve seen a whole industry is built upon a piece of open-source software!  Let’s keep on enjoying, appreciating  and maybe even contributing to GDAL and all other open-source software in the geospatial world!

#### Other Examples
- EPSG Database: As with many things the GIS industry has pioneered the development in this area with the foundational EPSG database showing how common data, in this case spatial reference system definitions, can enable interoperability across commercial, academic and governmental use. As an early example the EPSG license does not exactly meet the modern definition of open data (https://www.osgeo.org/about/what-is-open-data/)
 
 - openstreetmap.org
    
    - How/Why is it open: OpenStreetMap® is open data, licensed under the Open Data Commons Open Database License (ODbL) by the OpenStreetMap Foundation (OSMF). (https://www.openstreetmap.org/copyright)
    - What is the community effect of Open Street Map: Citizen science - contributions from everybody. One of the most detailed sources of local-scale map data available (https://www.openstreetmap.org/copyright). 
...
- How is data FAIR in the Cloud
  ...
  - Findalbe: STAC Catalouges, Find datasets (e.g. openEOplatform link: https://openeo.cloud/data-collections/ (*could be embedded*))
