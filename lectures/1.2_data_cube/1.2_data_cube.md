...
![Datacube Economy](https://github.com/EO-College/cubes-and-clouds/blob/main/lectures/1.2_data_cube/assets/datacubes_economy.png)
...
- **Seperated Collections:** Every satellite missions images form an own data cube, also called collection, with it's according metadata. Ideally cloud processing tools on the platform allow for easy data fusion of differenct collections.
...
::: Note: 
Remember, a data cube is not an EO cloud platform. It's only one part of it.
:::
...
## Quiz
- What is a two dimensional data cube: vector, feature, table
- Name the typical dimensions of an EO data cube: x, y, time, bands
- What could a geometry dimension be useful for: storing geometries (e.g. polygons, points) for data extraction,  defining the shape of the pixels, 
- Attach the resolutions to their dimensions: x - meter, kilometer; y - meter, kilometer, degrees; time - days, hours, years; bands - micrometers, color
- What are the advantages of data cubes in EO: EO data is multidimensional data cubes allow to represent this, Data cubes introduce an unnecessary layer of complexity that is not needed in EO, The file size of EO data is drastically reduced.
- What are the advantages of data cubes in cloud platforms: The user is not confronted with multiple files and file types, Different satellite missions can be stored in different data cubes that can be combined for analysis on the platform, subsetting operations become very powerful - the user only receives the extents he is interested in.
